...
ALTER TABLE public.missions
  ...
  ADD COLUMN IF NOT EXISTS closeout_checklist JSONB DEFAULT '{}',
...
CREATE TABLE IF NOT EXISTS public.mission_team_members (
  ...
  email TEXT,
  role TEXT NOT NULL,
  assigned_sections JSONB DEFAULT '[]',
...
DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE schemaname='public' AND tablename='mission_documents' AND policyname='Authenticated full access mission_documents') THEN
    CREATE POLICY "Authenticated full access mission_documents" ON public.mission_documents FOR ALL TO authenticated USING (true) WITH CHECK (true);
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE schemaname='public' AND tablename='mission_team_members' AND policyname='Authenticated full access mission_team_members') THEN
    CREATE POLICY "Authenticated full access mission_team_members" ON public.mission_team_members FOR ALL TO authenticated USING (true) WITH CHECK (true);
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE schemaname='public' AND tablename='mission_readiness' AND policyname='Authenticated full access mission_readiness') THEN
    CREATE POLICY "Authenticated full access mission_readiness" ON public.mission_readiness FOR ALL TO authenticated USING (true) WITH CHECK (true);
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE schemaname='public' AND tablename='mission_change_log' AND policyname='Authenticated full access mission_change_log') THEN
    CREATE POLICY "Authenticated full access mission_change_log" ON public.mission_change_log FOR ALL TO authenticated USING (true) WITH CHECK (true);
  END IF;
END $$;
