CREATE OR REPLACE FUNCTION public.developer_reset_all_mission_data()
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
...
DECLARE
  ...
  v_tables text[] := ARRAY[
    ...
    -- atlas mission-linked (FKs are SET NULL but we want a fully clean slate)
    'atlas_knowledge_objects','atlas_sources',
    -- graph & embeddings
    'graph_edges','graph_nodes','embeddings',
    -- final children of missions
...
BEGIN
  ...
  IF NOT v_is_admin THEN
    RAISE EXCEPTION 'Only platform admins can run developer reset';
  END IF;

  -- Loop through each table; skip silently if it doesn't exist.
  FOREACH v_table IN ARRAY v_tables LOOP
    ...
    EXCEPTION
      WHEN undefined_table THEN
        v_results := v_results || jsonb_build_object('table', v_table, 'skipped', 'no such table');
      WHEN OTHERS THEN
  ...
  -- Audit
  INSERT INTO public.olympus_audit_log (actor_user_id, action_type, action_summary, details)
  VALUES (
    v_caller,
...
-- Only platform admins should be able to invoke this RPC even via PostgREST.
REVOKE ALL ON FUNCTION public.developer_reset_all_mission_data() FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION public.developer_reset_all_mission_data() TO service_role;