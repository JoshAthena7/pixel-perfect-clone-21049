...
BEGIN
  ...
  -- Upsert by (mission_id, source_url) — synthetic source_url ties feed item to competitor row
  ...
  INSERT INTO public.intelligence_feed_items (
    ...
    NEW.mission_id, 'competitor', v_headline, v_summary, v_source,
...
DROP TRIGGER IF EXISTS trg_cascade_competitor_to_feed ON public.competitor_profiles;
CREATE TRIGGER trg_cascade_competitor_to_feed
AFTER INSERT OR UPDATE OF organization_name, competitor_type, likely_narrative,
  known_strengths, known_weaknesses, differentiation_strategy, known_relationships
ON public.competitor_profiles
FOR EACH ROW EXECUTE FUNCTION public.cascade_competitor_to_feed();

-- Also remove the feed item when the competitor is deleted
CREATE OR REPLACE FUNCTION public.cascade_competitor_delete_feed()
RETURNS TRIGGER
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
...
BEGIN
  DELETE FROM public.intelligence_feed_items
   WHERE mission_id = OLD.mission_id
     AND source_url = 'competitor:' || OLD.id::text;
  RETURN OLD;
END;
$$;

DROP TRIGGER IF EXISTS trg_cascade_competitor_delete_feed ON public.competitor_profiles;
CREATE TRIGGER trg_cascade_competitor_delete_feed
AFTER DELETE ON public.competitor_profiles
FOR EACH ROW EXECUTE FUNCTION public.cascade_competitor_delete_feed();

-- Backfill existing competitors into the feed
UPDATE public.competitor_profiles
   SET updated_at = now()
 WHERE true;
