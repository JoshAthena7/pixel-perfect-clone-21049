...
DECLARE
  v_state_code text;
BEGIN
  SELECT m.state_code INTO v_state_code FROM public.missions m WHERE m.id = p_mission_id;
  ...
  WHERE
    os.status::text IN ('approved','pushed')
    AND os.tier::text = ANY(p_include_tiers)
    AND (
      (os.tier::text = 'mission' AND os.mission_id = p_mission_id)
      OR (os.tier::text = 'state' AND os.state_code = v_state_code)
      OR (os.tier::text = 'platform')
    )
