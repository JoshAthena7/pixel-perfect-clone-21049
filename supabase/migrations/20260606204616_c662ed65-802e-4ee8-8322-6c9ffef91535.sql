DROP FUNCTION IF EXISTS public.developer_reset_all_mission_data(uuid);
DROP FUNCTION IF EXISTS public.developer_reset_all_mission_data();

CREATE FUNCTION public.developer_reset_all_mission_data(p_caller uuid)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  BEGIN
    INSERT INTO public.olympus_audit_log(actor_id, action, metadata)
    VALUES (v_caller, 'developer_reset_all_mission_data',
            jsonb_build_object('total_deleted', v_total, 'tables', v_results));
