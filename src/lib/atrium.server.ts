...
export async function getAtriumPayload(userId: string): Promise<AtriumPayload> {
  ...
  const totals = { wins: wonRows.length, awardedUsd: 0, peopleServed: 0, states: 0 };
  ...
  if (wonRows.length > 0) {
    ...
    const contributors: AtriumContributor[] = writerIds
      .map((wid) => ({ writerId: wid, displayName: nm.get(wid) ?? "Writer", contributionCount: counts.get(wid) ?? 0 }))
  ...
  const fmMap = new Map(((feedMissions.data ?? []) as Array<{ id: string; name: string }>).map((m) => [m.id, m.name]));
  ...
  for (const r of feed) {
    if (r.occurred_at >= cutoff && !liveMap.has(r.writer_id)) liveMap.set(r.writer_id, r.occurred_at);
  }
