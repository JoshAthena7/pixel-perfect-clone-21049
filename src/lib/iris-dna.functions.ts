...
async function callClaudeForDna(rfpText: string): Promise<MissionDna> {
  const apiKey = process.env.ANTHROPIC_API_KEY;
  if (!apiKey) throw new Error("ANTHROPIC_API_KEY is not configured");

  const body = rfpText.slice(0, 180_000); // Claude Sonnet handles huge context; cap for safety
  ...
  for (const model of models) {
    ...
    const parsed = JSON.parse(cleaned.slice(start, end + 1)) as MissionDna;
    return parsed;
  }
