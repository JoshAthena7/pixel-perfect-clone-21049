      ...
      <div className="divide-y divide-border">
        <div className="px-5 py-4">
          <div className="flex items-center justify-between gap-3">
            <div>
              <div className="text-sm font-medium text-foreground">Refresh Horizon Feed</div>
              <div className="text-xs text-muted-foreground">Pull fresh items from Federal Register, CMS, HHS, Medicaid.gov.</div>
            </div>
            <button
              onClick={runMarketIntel}
              disabled={running !== null}
              className="inline-flex items-center gap-1.5 rounded-md border border-border bg-background px-3 py-1.5 text-sm hover:bg-muted disabled:opacity-50"
            >
              <Zap className="h-3.5 w-3.5" />
              {running === "market" ? "Running…" : "Run now"}
            </button>
          </div>
        </div>
        <div className="px-5 py-4">
          <div className="mb-3 text-sm font-medium text-foreground">Parse RFP → Question Records</div>
          {rfpDocs.length === 0 ? (
            <div className="text-xs text-muted-foreground">No RFPs uploaded yet.</div>
          ) : (
            <ul className="space-y-2">
              {rfpDocs.map((d) => (
                <li key={d.id} className="flex items-center justify-between gap-3 rounded-md border border-border bg-background px-3 py-2">
                  ...
                  <button
                    onClick={() => runRfpParse(d.id, d.name)}
                    disabled={running !== null}
