  ...
  errorComponent: ({ error, reset }) => {
    ...
    return (
      <div className="mx-auto max-w-xl px-6 py-16 text-center">
        <h2 className="text-lg font-semibold">Couldn't load the cockpit</h2>
...
function MissionCockpitPage() {
  const { missionId } = Route.useParams();
  const qc = useQueryClient();

  const { data: me, isLoading: meLoading } = useQuery({
    queryKey: ["mc-cockpit-me"],
    queryFn: async () => {
  ...
  const { data: allQuestions = [], isLoading: qLoading } = useQuery<Q[]>({
    queryKey: ["mc-cockpit-questions", missionId],
    queryFn: async () => {
